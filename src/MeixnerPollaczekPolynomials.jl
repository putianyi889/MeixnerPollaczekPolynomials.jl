...
import RationalVals: RationalVal, IntegerVal
...
function getindex(w::MeixnerPollaczekWeight, x::Number)
    ...
    abs2(gamma(w.λ + x * im)) * exp(x * (IntegerVal{2}() * w.ϕ - π))
end
...
==(A::MeixnerPollaczek, B::MeixnerPollaczek) = A.λ == B.λ && A.ϕ == B.ϕ
...
jacobimatrix(S::MeixnerPollaczek) = Tridiagonal(RationalVal{1,2}() * csc(S.ϕ) * oneto(∞), -cot(S.ϕ) * (S.λ:∞), RationalVal{1,2}() * csc(S.ϕ) * (IntegerVal{2}()*S.λ:∞))
